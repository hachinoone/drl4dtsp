...
def get_costs(dataset, pi, state, mat):
    ...
    bdd = mat.var[state.prev_a.squeeze() * mat.n_c]
    add = torch.clamp(mat.__getd__(ind, state.prev_a, depots, state.lengths) + torch.clamp(torch.randn(state.prev_a.size(0), device=device) * bdd, min=-0.01, max=0.01), min=0)
    return state.lengths.squeeze() + add, None
...
class StateTSP(NamedTuple):
    ...
    def update(self, selected, mat, input):
        ...
        add = torch.clamp(mat.__getd__(ind, self.prev_a, prev_a, self.lengths).unsqueeze(1) + torch.clamp(torch.randn(prev_a.size(0), 1, device=device) * bdd, min=-0.01, max=0.01), min=0)
